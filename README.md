...
- 2026-09-23 문서·코드 검토: README/ARCHITECTURE/CLAUDE.md의 주장을 코드와 대조해 부정확한 문장 20여 곳을 고치고, 코드 결함 17건을 회귀 테스트와 함께 수정(`tests/test_md_review_findings_regressions.py`); Ubuntu의 ngspice-42 공유 라이브러리로 SPICE 러너를 실측(`ngSpice_Reset` 없음·solver 안내문을 stderr로 출력 → 선택적 바인딩·정보성 stderr 분류; 숫자 파서 모델은 표본 500개 일치); 설계 해시가 경로·검증 상태를 담지 않도록 모델별 design view 도입; RELEASE가 tool 없는 PASS·다른 IR 버전의 PASS를 증거로 세지 않음.
