"""PCBAgent + the deterministic grid placer + the maze router in the PLACEMENT stage.
...
extending to 2.4 mm - extent 2.4 x 1.2 mm) and, for the divider fixture, on
the synthetic template library of ``tests/test_circuit_templates.py``. The
two tests that need ``kicad-cli`` and the installed KiCad libraries skip
elsewhere and run against the real binary on the Windows PC: one asserts
the honest verdict on a board left unrouted (``pcb.routing=skip``: DRC
FAILs with ``unconnected_items``), the other states the expectation for the
maze-routed divider (0 unconnected items, no clearance / width / via
violation) - an expectation, never measured yet.
"""
...
from ai_eda.agents.pcb import PLACEMENT_KEY, ROUTING_KEY, UNROUTED_NOTE
...
from ai_eda.tools.routing import ROUTER_ID, ROUTER_VERSION
...
def _routing_checks(ir: CircuitIR, tmp_path: Path, lib: KicadLibrary | None) -> dict[str, object]:
    ...
    tools = {"kicad_library": lib} if lib is not None else {}
    return {r.check_id: r for r in default_registry.get("pcb.routing").validate(ir, ValidationContext(workdir=tmp_path, tools=tools))}
...
def test_agent_proposes_only_when_nothing_is_placed(tmp_path: Path, lib: KicadLibrary):
    # ir.pcb None -> one whole-pcb proposal (placement + copper) that validates through apply_proposals
    ir = parts_ir(tmp_path, lib)
    res = _run_agent(ir, tmp_path, lib)
    ...
    assert res.proposals[0].payload.tracks and not any("unconnected_items" in n for n in res.notes)  # routed: the DRC warning belongs to an unrouted board only
    Orchestrator.apply_proposals(ir, res.proposals)
    ...
    res = _run_agent(ir, tmp_path, lib)
    assert len(res.proposals) == 1 and res.proposals[0].payload.tracks == [] and res.notes[0].endswith(f"; {UNROUTED_NOTE}")
    assert res.notes[1].startswith("not routed: cannot route: this router knows only ['F.Cu', 'B.Cu'], ir.pcb.layers also has ['In1.Cu', 'In2.Cu']")
    Orchestrator.apply_proposals(ir, res.proposals)
    assert ir.pcb.layers == layers and ir.pcb.manufacturing.fab == "JLCPCB" and ir.pcb.manufacturing.min_track_width_mm.value == 0.127
    assert len(ir.pcb.placements) == 5 and ir.pcb.outline == BoardOutline(width_mm=16.6, height_mm=7.4) and ir.pcb.tracks == []
    ...
    assert ir.pcb.tracks and all(100.0 <= x <= 125.0 and 60.0 <= y <= 72.0 for t in ir.pcb.tracks for x, y in (t.start, t.end))  # routed inside the user outline
...
def test_agent_places_and_routes_in_one_proposal(tmp_path: Path, lib: KicadLibrary):
    """Placement + copper in the one ``pcb`` payload: routing.maze provenance on every track, the validator's connectivity PASSes on it."""
    ir = parts_ir(tmp_path, lib)
    res = _run_agent(ir, tmp_path, lib)
    [proposal] = res.proposals
    payload: PCBDesign = proposal.payload
    assert len(payload.placements) == 5 and payload.tracks and payload.vias == [] and payload.zones == []
    for item in [*payload.tracks, *payload.vias]:
        prov = item.provenance
        assert prov.kind is ProvenanceKind.DERIVED and prov.tool == ROUTER_ID == "routing.maze" and prov.tool_version == ROUTER_VERSION and not prov.needs_verification
        assert prov.derived_from[0] == f"net:{item.net}" and prov.derived_from[-1].startswith("params:grid=0.25,width=0.4,clearance=0.25,via=0.8/0.4,edge=0.3")
    assert all(t.layer in ("F.Cu", "B.Cu") and t.width_mm == 0.4 for t in payload.tracks)
    # the notes and the description name the router, its version, the counts, the copper length and the vias; never "clean"
    ...
    assert checks[CONNECTIVITY_CHECK].status is S.PASS and "not DRC" in checks[CONNECTIVITY_CHECK].message
    assert [r["net"] for r in checks[CONNECTIVITY_CHECK].details["nets"] if r["status"] == "PASS"] == ["N0", "N1", "N2", "N3"]
    assert checks[CLEARANCE_CHECK].status is S.NOT_VERIFIED and "no clearance limit" in checks[CLEARANCE_CHECK].message
    ir.pcb.manufacturing = ManufacturingConstraints(min_clearance_mm=assumption(0.25, note="the router's own clearance, as a limit"))
    assert _routing_checks(ir, tmp_path, lib)[CLEARANCE_CHECK].status is S.PASS
    # the reviewer accepts the routed board: every track says where it came from
    art = PCBCompiler().compile(ir, CompileContext(workdir=tmp_path, tools={"kicad_library": lib}))
    ir.artifacts[ArtifactKind.PCB] = art
    areas = {r.check_id: r for r in IndependentReviewer(tools={"kicad_library": lib}).review(ir, tmp_path).results}
    assert areas[ReviewArea.IR_VS_PCB].status is S.PASS, areas[ReviewArea.IR_VS_PCB].message
    assert sexpr.find_all(sexpr.parse_file(Path(art.path)), "segment") and len(sexpr.find_all(sexpr.parse_file(Path(art.path)), "segment")) == len(ir.pcb.tracks)


def test_existing_placements_without_copper_are_routed_and_never_moved(tmp_path: Path, lib: KicadLibrary):
    ir = parts_ir(tmp_path, lib)
    Orchestrator.apply_proposals(ir, _run_agent(ir, tmp_path, lib, **{ROUTING_KEY: "skip"}).proposals)
    placed = [p.model_copy(deep=True) for p in ir.pcb.placements]
    assert ir.pcb.tracks == [] and ir.pcb.vias == []
    res = _run_agent(ir, tmp_path, lib)
    [proposal] = res.proposals
    assert res.notes[0] == "not placed: ir.pcb already has 5 placement(s); the agent never replaces a layout" and res.notes[1].startswith(f"{ROUTER_ID} {ROUTER_VERSION}: 4 net(s) routed")
    assert proposal.description == res.notes[1] and proposal.payload.placements == placed and proposal.payload.tracks and proposal.payload.outline == ir.pcb.outline
    Orchestrator.apply_proposals(ir, res.proposals)
    assert ir.pcb.placements == placed and ir.pcb.tracks and _routing_checks(ir, tmp_path, lib)[CONNECTIVITY_CHECK].status is S.PASS
    # a hand-placed board whose parts the library cannot find is neither moved nor routed: the note says why
    ir = parts_ir(tmp_path, lib)
    Orchestrator.apply_proposals(ir, _run_agent(ir, tmp_path, lib, **{ROUTING_KEY: "skip"}).proposals)
    ir.component("R2").footprint.name = "Missing"
    res = _run_agent(ir, tmp_path, lib)
...
def test_an_unroutable_board_gets_the_placement_only_and_never_half_a_route(tmp_path: Path, lib: KicadLibrary):
    """A clearance limit wider than the pitch fences every pad: no net can be routed, the placement is proposed alone, each net's reason is noted."""
    ir = parts_ir(tmp_path, lib)
    ...
    # with the limit gone the same placements route on the next run (routing only)
    ir.pcb.manufacturing = ManufacturingConstraints()
    res = _run_agent(ir, tmp_path, lib)
    assert len(res.proposals) == 1 and res.proposals[0].payload.tracks and res.notes[0].startswith("not placed: ir.pcb already has 5 placement(s)")
...
def test_existing_copper_is_never_replaced(tmp_path: Path, lib: KicadLibrary):
    """A placed board with any copper (a hand-drawn track, a via, a zone) gets no proposal at all, whatever the copper is worth."""
    ir = parts_ir(tmp_path, lib)
    Orchestrator.apply_proposals(ir, _run_agent(ir, tmp_path, lib, **{ROUTING_KEY: "skip"}).proposals)
    hand = Provenance(kind=ProvenanceKind.USER_REQUIREMENT, note="drawn by hand")
    ir.pcb.tracks = [Track(net="N0", layer="F.Cu", start=(1.0, 1.0), end=(5.0, 1.0), width_mm=0.25, provenance=hand)]
    before = copy.deepcopy(ir.pcb)
    res = _run_agent(ir, tmp_path, lib)
    assert res.proposals == [] and res.notes == [
        "not placed: ir.pcb already has 5 placement(s); the agent never replaces a layout",
    ...
    ir.pcb.tracks = []
    ir.pcb.vias = [Via(net="N0", x_mm=3.0, y_mm=3.0, drill_mm=0.4, diameter_mm=0.8, provenance=hand)]
    res = _run_agent(ir, tmp_path, lib)
    assert res.proposals == [] and res.notes[1] == "not routed: ir.pcb already has copper (0 track(s), 1 via(s), 0 zone(s)); the agent never replaces copper"
...
def test_placement_answer_is_a_control_key_and_skip_proposes_nothing(tmp_path: Path, lib: KicadLibrary):
    ...
    for answer, placed in (("skip", False), ("yes", True)):
        ...
        assert (ir.pcb is not None and bool(ir.pcb.tracks)) is placed  # placed means routed too
        assert (ir.validation.latest(CONNECTIVITY_CHECK).status is S.PASS) is placed  # unplaced: NOT_APPLICABLE
        assert state.outcome(Stage.IR_BUILD).status is not S.FAIL


def test_routing_answer_is_a_control_key_and_skip_leaves_the_placement_without_copper(tmp_path: Path, lib: KicadLibrary):
    assert ROUTING_KEY == "pcb.routing" and ROUTING_KEY in CONTROL_KEYS
    # skip: the placement alone, the DRC warning back in the note
    ir = parts_ir(tmp_path, lib)
    ...
    ir = parts_ir(tmp_path / "skip", lib)
    state = Orchestrator(_ctx(tmp_path / "skip", lib, **{ROUTING_KEY: "skip"})).run(ir)
    ...
    out = state.outcome(Stage.PLACEMENT)
    assert out.status is S.NOT_VERIFIED and "routing skipped by answer" in out.message and "unconnected_items" in out.message
    assert len(ir.pcb.placements) == 5 and ir.pcb.tracks == []
    connectivity = ir.validation.latest(CONNECTIVITY_CHECK)
    assert connectivity.status is S.FAIL and connectivity.ir_hash == ir.content_hash() and "not DRC" in connectivity.message
    assert [r["net"] for r in connectivity.details["nets"] if r["status"] == "FAIL"] == ["N0", "N1", "N2", "N3"]
    assert state.outcome(Stage.IR_BUILD).status is S.FAIL and not state.blocked and state.outcomes[-1].stage is Stage.RELEASE
    assert state.outcome(Stage.RELEASE).status is S.FAIL and CONNECTIVITY_CHECK in state.outcome(Stage.RELEASE).message and run_exit_code(state) == 1
...
def test_placement_stage_runs_before_ir_build_and_keeps_validator_hashes_fresh(tmp_path: Path, lib: KicadLibrary):
    ...
    assert f"; {ROUTER_ID} {ROUTER_VERSION}: 4 net(s) routed" in out.message and out.questions == []
    assert ir.pcb.tracks and state.outcome(Stage.PCB).status is S.PASS and ArtifactKind.PCB in ir.artifacts
    assert state.outcome(Stage.DRC).status is S.NOT_VERIFIED  # kicad-cli decides, and it is not here
    # every validator result is about the placed and routed design: nothing is stale at RELEASE
    current = ir.content_hash()
    stamped = [r for r in ir.validation.results if r.check_id.startswith("ir.") or r.check_id.startswith("pcb.routing.")]
    assert stamped and all(r.ir_hash == current for r in stamped)
    assert ir.validation.latest(CONNECTIVITY_CHECK).status is S.PASS and ir.validation.latest(CLEARANCE_CHECK).status is S.NOT_VERIFIED
...
def test_placement_stage_routes_the_divider_and_ir_build_passes_connectivity(tmp_path: Path):
    """The divider fixture (hand placements, no copper) on the synthetic template library: PLACEMENT routes it, IR_BUILD proves the IR copper connects."""
    lib = template_library(tmp_path / "kicad")
    ir = divider_with_connector_ir(tmp_path, lib)
    assert all(c.footprint.verified for c in ir.components) and ir.pcb.tracks == []
    placed = [p.model_copy(deep=True) for p in ir.pcb.placements]
    state = Orchestrator(AgentContext(workdir=tmp_path, tools={"kicad_library": lib}, answers=SCOPE_ANSWERS)).run(ir, stop_after=Stage.IR_BUILD)
    out = state.outcome(Stage.PLACEMENT)
    assert out.status is S.NOT_VERIFIED and out.message.startswith("1 proposal(s) applied, nothing verified; not placed: ir.pcb already has 3 placement(s)")
    assert f"; {ROUTER_ID} {ROUTER_VERSION}: 3 net(s) routed on F.Cu/B.Cu" in out.message
    assert ir.pcb.placements == placed and ir.pcb.tracks and {t.net for t in ir.pcb.tracks} == {"VIN", "VOUT", "GND"}
    connectivity = ir.validation.latest(CONNECTIVITY_CHECK)
    assert connectivity.status is S.PASS and connectivity.ir_hash == ir.content_hash() and connectivity.tool == "pcb.routing"
    assert {r["net"]: r["status"] for r in connectivity.details["nets"]} == {"VIN": "PASS", "VOUT": "PASS", "GND": "PASS"}
    assert ir.validation.latest(CLEARANCE_CHECK).status is S.NOT_VERIFIED  # no fab limit in the fixture
    assert state.outcome(Stage.IR_BUILD).status is not S.FAIL and not state.blocked
...
def test_placed_but_unrouted_divider_fails_real_drc_with_unconnected_items(tmp_path: Path):
    """The honest verdict on a KiCad machine for a board left unrouted (``pcb.routing=skip``): IR_BUILD and DRC FAIL (unconnected), exit code 1.

    A components-only IR gets a placed board without copper;
    ``pcb.routing.connectivity`` FAILs at IR_BUILD (IR geometry), DRC FAILs
    with ``unconnected_items``. The expected count is derived from the IR
    (one ratsnest line per pad beyond the first in each net: VIN 1 + VOUT 2
    + GND 1 = 4 for the fixture); the run on the Windows PC confirms it.
    Nothing here claims a DRC-clean board.
    """
    ir = divider_with_connector_ir(tmp_path, _lib)
    ir.pcb = None  # components only: no outline, no placements, no copper
    ctx = AgentContext(workdir=tmp_path, tools={"kicad_library": _lib, "kicad_cli": _kicad}, answers={**SCOPE_ANSWERS, ROUTING_KEY: "skip"})
    state = Orchestrator(ctx).run(ir)
    assert not state.blocked
    placement = state.outcome(Stage.PLACEMENT)
    assert placement.status is S.NOT_VERIFIED and "1 proposal(s) applied" in placement.message and "routing skipped by answer" in placement.message
    assert [p.component_ref for p in ir.pcb.placements] == ["J1", "R1", "R2"] and ir.pcb.tracks == []
    assert state.outcome(Stage.IR_BUILD).status is S.FAIL and ir.validation.latest(CONNECTIVITY_CHECK).status is S.FAIL
...
@needs_kicad
def test_maze_routed_divider_passes_real_drc_connectivity_and_clearance(tmp_path: Path):
    """EXPECTATION, never measured yet: the maze-routed divider's real DRC reports 0 ``unconnected_items`` and no clearance / track width / via violation.

    This is what the router's geometry promises (every net connected through
    IR copper at 0.4 mm width, 0.25 mm clearance, 0.8 / 0.4 mm vias, 0.3 mm
    from the edge) translated into KiCad's verdict on the compiled board.
    It has not been run on any machine with kicad-cli; the Windows PC is
    where it is to be measured, and a FAIL there is a finding about the
    router, not about DRC. Other violation types (courtyard, silk, ...) are
    reported, not asserted: they are the placer's business.
    """
    ir = divider_with_connector_ir(tmp_path, _lib)
    ir.pcb = None  # components only: PLACEMENT places and routes
    ctx = AgentContext(workdir=tmp_path, tools={"kicad_library": _lib, "kicad_cli": _kicad}, answers=SCOPE_ANSWERS)
    state = Orchestrator(ctx).run(ir)
    placement = state.outcome(Stage.PLACEMENT)
    assert placement.status is S.NOT_VERIFIED and f"{ROUTER_ID} {ROUTER_VERSION}: 3 net(s) routed" in placement.message, placement.message
    assert ir.pcb.tracks and ir.validation.latest(CONNECTIVITY_CHECK).status is S.PASS
    assert state.outcome(Stage.PCB).status is S.PASS, state.outcome(Stage.PCB).message
    drc = state.outcome(Stage.DRC)
    res = ir.validation.latest("kicad.drc")
    assert res is not None and res.is_tool_backed and res.artifact_hash == ir.artifacts[ArtifactKind.PCB].content_hash
    unconnected = res.details["unconnected_items"]
    types = sorted({v.get("type") for v in res.details["errors"] + res.details["warnings"]})
